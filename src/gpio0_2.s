...
/*NEW
* Subrutina diseñada por el alumno Josué Cifuentes carné No. 15275
* Válida para los pines GPIO 0 - 31.
* GetGpio
* Parámetros: R0 <- número de puerto/pin
* Salidas: R0 -> 1 (HIGH) | 2 (LOW)
*/
.global GetGpio
GetGpio:
	ldr r1,=myloc
	ldr r2,[r1] @Se obtiene la dirección de la memoria virtual.
	ldr r1,[r2,#0x34] @Se obtiene la dirección de GPIO para lectura de pines.
	mov r2,#1
	lsl r2,r0 @Se deja el uno en la posición del pin que se quiere verificar.
	and r0,r1,r2 @Si el resultado es distinto de cero, el botón esta siendo apachado.
	mov pc,lr
